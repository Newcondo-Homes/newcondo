...
class SMSService {
    ...
    async sendBulkSMS(messages) {
        const results = await Promise.allSettled(messages.map(msg => this.sendSMS(msg)));
        return {
    ...
    async checkBalance() {
        try {
            const response = await axios_1.default.get(`${this.baseUrl}/get-balance?api_key=${this.apiKey}`);
            return {
