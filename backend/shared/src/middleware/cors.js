...
exports.preflightHandler = exports.corsMiddleware = void 0;
...
function isAllowedOrigin(origin) {
    ...
    if (env !== 'production') {
        if (DEVELOPMENT_ORIGINS.includes(origin))
            return true;
        if (RENDER_PREVIEW_PATTERN.test(origin))
            return true;
 ...
 */
exports.corsMiddleware = (0, cors_1.default)(corsOptionsDelegate);
/**
 ...
 */
exports.preflightHandler = (0, cors_1.default)(corsOptionsDelegate);
/**
 * Convenience function for services that need to override CORS options at
 * mount-time (e.g. the combined-backend gateway).
 */
function createCorsMiddleware(extraOrigins = []) {
    const overrideDelegate = (req, callback) => {
        const origin = req.headers.origin;
        if (!origin)
            return callback(null, { origin: false });
    ...
    return (0, cors_1.default)(overrideDelegate);
}
/**
