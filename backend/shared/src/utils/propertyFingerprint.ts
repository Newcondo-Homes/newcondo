...
import { VALIDATION_THRESHOLDS } from '../constants/boundaries';

// DEFINED LOCALLY: Explicit structure for the property fingerprint
...
export function generatePropertyFingerprint(
  ...
  const boundingBox = getBoundingBox([bounds.southWest, bounds.northEast]);
...
export function detectDuplicateProperties(
  newBounds: RectangleBounds,
  existingMasks: Array<{ mask: RectangleBounds; fingerprint: PropertyFingerprint; propertyId: string; address: string }>,
  newAddress: string,
  strictMode = false
): DuplicateDetectionResult {
  ...
  } as unknown as DuplicateDetectionResult;
}
...
export function calculateSimilarity(fp1: PropertyFingerprint, fp2: PropertyFingerprint): number {
  ...
  const maxDistance = VALIDATION_THRESHOLDS.MAX_DISTANCE_FROM_DECLARED_LOCATION || 1000;
 ...
 */
export function calculateOverlapPercentage(bounds1: RectangleBounds, bounds2: RectangleBounds): number {
  if (!doPolygonsOverlap(bounds1, bounds2)) {
    return 0;
  }

  const overlapArea = calculateBoundingBoxOverlap(bounds1, bounds2);
  const area1 = calculatePolygonArea(bounds1);
  const area2 = calculatePolygonArea(bounds2);
...
function isStrictDuplicate(fp1: PropertyFingerprint, fp2: PropertyFingerprint, overlapPercentage: number): boolean {
  const strictOverlap = VALIDATION_THRESHOLDS.DEFINITE_DUPLICATE_THRESHOLD || 75;
  return (
    overlapPercentage >= strictOverlap &&
...
function isLooseDuplicate(fp1: PropertyFingerprint, fp2: PropertyFingerprint, overlapPercentage: number): boolean {
  const looseOverlap = VALIDATION_THRESHOLDS.SIGNIFICANT_OVERLAP_THRESHOLD || 25;
  return (
    overlapPercentage >= looseOverlap ||
    calculateSimilarity(fp1, fp2) >= 0.55
  );
}

function normalizeCoordinatesForHashing(bounds: RectangleBounds): GeoLocationCoordinates[] {
...
function getGeographicZone(center: GeoLocationCoordinates): string {
  const gridSize = 0.01;
...
function calculateBoundingBoxOverlap(bbox1: RectangleBounds, bbox2: RectangleBounds): number {
...
function generateDuplicateReason(
  ...
  if (overlapPercentage >= (VALIDATION_THRESHOLDS.SIGNIFICANT_OVERLAP_THRESHOLD || 25)) {
