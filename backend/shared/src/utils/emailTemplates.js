...
// ============================================================
const business_1 = require("../constants/business");
...
/* ---------- building blocks ---------- */
const btn = (label, url) => `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:26px auto 6px"><tr><td style="border-radius:999px;background:${C.ink}"><a href="${url}" style="display:inline-block;padding:13px 30px;font-family:${FONT};font-size:14px;font-weight:600;color:${C.cream};text-decoration:none;border-radius:999px">${label}</a></td></tr></table>`;
const kvRows = (rows) => `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:18px 0 4px">${rows
    ...
    .join("")}</table>`;
const amountBlock = (label, amount, tone = "neutral") => `<div style="text-align:center;margin:22px 0 6px"><div style="font-family:${FONT};font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:${C.sub}">${label}</div><div style="font-family:${MONO};font-size:32px;font-weight:600;letter-spacing:-.02em;color:${tone === "in" ? C.green : C.ink};margin-top:6px">${amount}</div></div>`;
const otpBlock = (code) => `<div style="text-align:center;margin:24px 0 8px"><div style="display:inline-block;background:${C.sunken};border:1px solid ${C.hair};border-radius:16px;padding:18px 34px;font-family:${MONO};font-size:34px;font-weight:600;letter-spacing:.35em;color:${C.ink}">${code}</div></div>`;
const notice = (html, tone = "info") => {
...
const ngn = (n) => `\u20a6${Math.abs(n).toLocaleString("en-NG")}`;
...
<tr><td style="padding:0 6px 18px"><span style="font-family:${FONT};font-size:22px;font-weight:700;letter-spacing:-.04em;color:${C.ink}">${business_1.COMPANY.wordmark}</span></td></tr>
...
This email was sent by Newcondo (${business_1.COMPANY.domain}). Never share your OTP or password; Newcondo staff will never ask for them.<br>
<a href="https://${business_1.COMPANY.domain}" style="color:${C.sub}">Visit ${business_1.COMPANY.domain}</a> · <a href="https://${business_1.COMPANY.domain}/support" style="color:${C.sub}">Support</a>
</td></tr>
</table></td></tr></table></body></html>`;
}
const dash = (path = "/dashboard") => `https://${business_1.COMPANY.domain}${path}`;
/* ============================================================
