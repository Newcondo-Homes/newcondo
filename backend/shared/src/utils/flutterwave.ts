...
export const encryptPaymentData = (data: string): string => {
  const secretKey = flutterwaveConfig.FLUTTERWAVE_ENCRYPTION_KEY;
  const cipher = crypto.createCipheriv('des-ede3-ecb', secretKey, Buffer.alloc(0));
...
export const decryptPaymentData = (encryptedData: string): string => {
  ...
  const decipher = crypto.createDecipheriv('des-ede3-ecb', secretKey, Buffer.alloc(0));
