...
interface AgentPerformanceMetrics {
  ...
  customerRatings?: number[]; // Array of ratings (1-5)
  disputedJobs: number;
}
...
function calculateCompletionRate(
  ...
  if (rate >= 0.9) return 4.5;
  if (rate >= 0.85) return 4.0;
  if (rate >= 0.8) return 3.5;
  if (rate >= 0.75) return 3.0;
  if (rate >= 0.7) return 2.5;
  if (rate >= 0.6) return 2.0;
  if (rate >= 0.5) return 1.5;
  return 1.0;
}
...
function calculateTimelinessScore(
  ...
  const totalCompletions =
    metrics.onTimeCompletions + metrics.lateCompletions;
  ...
  if (onTimeRate >= 0.95) return 5.0;
  if (onTimeRate >= 0.9) return 4.5;
  if (onTimeRate >= 0.85) return 4.0;
  if (onTimeRate >= 0.8) return 3.5;
  if (onTimeRate >= 0.75) return 3.0;
  if (onTimeRate >= 0.7) return 2.5;
  if (onTimeRate >= 0.6) return 2.0;
  return 1.0;
}
...
function calculateReliabilityScore(
  ...
  const problematicJobs =
    metrics.cancelledJobs + metrics.expiredJobs + metrics.disputedJobs;
  const problematicRate = problematicJobs / metrics.totalMarkingJobs;

  // Lower problematic rate = higher score
  if (problematicRate <= 0.02) return 5.0;
  if (problematicRate <= 0.05) return 4.5;
  if (problematicRate <= 0.08) return 4.0;
  if (problematicRate <= 0.1) return 3.5;
  if (problematicRate <= 0.15) return 3.0;
  if (problematicRate <= 0.2) return 2.5;
  if (problematicRate <= 0.25) return 2.0;
  return 1.0;
}
 ...
 */
function calculateCustomerRatingScore(
  metrics: AgentPerformanceMetrics
): number {
...
function getScoreDescription(score: number): string {
  if (score >= 4.5) return 'Excellent - Highly reliable agent';
  if (score >= 4.0) return 'Very Good - Consistently reliable';
  if (score >= 3.5) return 'Good - Generally reliable';
  if (score >= 3.0) return 'Average - Acceptable performance';
  if (score >= 2.0) return 'Below Average - Needs improvement';
 ...
 */
export function getPriorityMultiplier(score: number): number {
  if (score >= 4.5) return 1.5; // Platinum agents get 50% priority boost
  if (score >= 4.0) return 1.3; // Gold agents get 30% priority boost
  if (score >= 3.5) return 1.2; // Silver+ agents get 20% priority boost
  if (score >= 3.0) return 1.0; // Average agents - no boost
  if (score >= 2.5) return 0.9; // Below average - slight penalty
 ...
 */
export function updateMetricsAfterJob(
 ...
 */
export function getRecommendedActions(score: number): string[] {
