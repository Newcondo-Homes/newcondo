...
import { 
  ...
  estimateTravelTime 
} from '../constants/proximityRadius';
 ...
 */
export const geoProxCalculateDistance = (
  point1: Coordinates,
  point2: Coordinates
): number => {
...
export const isWithinRadius = (
  point1: Coordinates,
  point2: Coordinates,
  radiusKm: number = DEFAULT_PROXIMITY_RADIUS // FIXED
): boolean => {
  const distance = geoProxCalculateDistance(point1, point2);
...
export const getProximity = (
  ...
  maxRadius: number = DEFAULT_PROXIMITY_RADIUS // FIXED
): ProximityResult => {
  const distance = geoProxCalculateDistance(agentLocation, propertyLocation);
 ...
 */
export const geoGetBoundingBox = (
  center: Coordinates,
  radiusKm: number
): {
  minLat: number;
