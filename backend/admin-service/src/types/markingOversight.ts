...
export interface MarkingJobIssue {
  id: string;
  type: IssueType;
...
export interface AdminFlag {
  id: string;
  type: 'SUSPICIOUS_ACTIVITY' | 'QUALITY_CONCERN' | 'TIMING_ISSUE' | 'PAYMENT_ISSUE';
  reason: string;
  flaggedAt: Date;
...
export interface MarkingJobReview {
  jobId: string;
  adminId: string;
  approved: boolean;
  qualityScore: number;
...
export interface ReviewAction {
  type: 'RELEASE_PAYMENT' | 'WITHHOLD_PAYMENT' | 'REASSIGN_JOB' | 'SUSPEND_AGENT' | 'REQUEST_REWORK' | 'REFUND_CUSTOMER';
  details: string;
  amount?: number;
}

export interface MarkingJobIntervention {
  jobId: string;
  adminId: string;
  interventionType: 'REASSIGN' | 'CANCEL' | 'EXTEND_DEADLINE' | 'MANUAL_COMPLETION' | 'DISPUTE_RESOLUTION';
  reason: string;
