...
class VerificationService {
  async sendEmailVerification(userId: string, email: string) {
    try {
      ...
      if (user.emailVerified) {
        throw new AuthError('Email is already verified', 'EMAIL_ALREADY_VERIFIED', 400);
      }

      // Generate and send OTP
      const otp = await otpService.generateOTP(email, OTPType.EMAIL_VERIFICATION);
      
      // Send email with OTP
      const emailSent = await emailService.sendVerificationEmail(email, otp.code, user.name || 'User');
  ...
  async verifyEmail(userId: string, email: string, code: string) {
    try {
      // Verify OTP
      const isValidOTP = await otpService.verifyOTP(email, code, OTPType.EMAIL_VERIFICATION);
      
      if (!isValidOTP) {
        throw new AuthError('Invalid or expired verification code', 'INVALID_OTP', 400);
      }
  ...
  async resendEmailVerification(userId: string, email: string) {
    try {
      ...
      if (recentOTP) {
        const nextResendTime = new Date(recentOTP.createdAt.getTime() + 2 * 60 * 1000);
        throw new AuthError(
