...
export interface Commission {
  ...
  // Commission split fields (set by CommissionService.distributeCommission)
  agentCommission: number | null;   // listing agent + sub-agent combined
  platformFee: number | null;       // NewCondo's share
  ownerAmount: number | null;       // property owner's share
 
  // Release tracking
  isReleased: boolean;
  releasedAt: string | null;        // ISO date string from API
...
export interface GetCommissionsResponse {
  ...
  // Server-side aggregates (filtered by query params)
...
// ─────────────────────────────────────────────
// Summary response — mirrors CommissionService
// getUserCommissionSummary + dashboard extras
// ─────────────────────────────────────────────
...
export interface GetCommissionSummaryResponse {
  ...
  // Breakdown by agent role (from CommissionService breakdown logic)
  listingAgentEarnings: number;   // sum of agentCommission where user = listing agent
  subAgentEarnings: number;       // sum of agentCommission where user = sub-agent
  markingServiceEarnings: number; // sum of payments where paymentType = PROPERTY_MARKING
