...
export function SectionHead({
  ...
  return (
    <div className={cx(center ? "max-w-[920px] mx-auto text-center" : "max-w-[920px]", "mb-14", className)}>
      ...
      <Reveal as="h2" variants={vLead} viewport={lateViewport} className="nc-h2">
        {title}
      </Reveal>
      {lead && (
        <Reveal as="p" variants={vLead} viewport={lateViewport} className={cx("nc-lead mt-5", center && "mx-auto")}>
          {lead}
        </Reveal>
      )}
      {children}
