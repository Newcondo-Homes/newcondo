...
/* ============================================================
   OnboardingForm  (formerly RegisterForm)

   Presentation rebuilt to match the approved onboarding preview 1:1
   (bare role selector under "What brings you to Newcondo?", then a
   compact two-column details form with icon inputs, a custom checkbox,
   and pill buttons). All of the original behaviour is preserved —
   state shape, validation rules, the useRegister mutation, the social
   sign-in handlers, and the toast calls. After the account is created
   it hands control back to the onboarding flow via `onRegistered(...)`.
   ============================================================ */
...
/* Shared input styling — matches the preview's .inp */
const INPUT_BASE =
...
export default function OnboardingForm({
  ...
  /** Pre-select a role (e.g. PROPERTY_OWNER when the user clicked "List your property"). */
  initialRole?: UserType;
  /** Called after the account is created — advances the onboarding flow to the plan step. */
  onRegistered?: (profile: OnboardingProfile) => void;
} = {}) {
  const router = useRouter();
  const [step, setStep] = useState<"user-type" | "details">(
    initialRole ? "details" : "user-type"
  );
      ...
      },
      {
        onSuccess: () => {
          ...
          localStorage.removeItem(
            `otp_expiry_${formData.email.trim().toLowerCase()}_EMAIL_VERIFICATION`
          );

          /* ── Continue the onboarding flow → plan step ──
             If you want email verification BEFORE the plan step, route to
             /verify-otp here instead and resume onboarding afterwards:
  ...
  /* ============================================================
     STEP 1 — account type (bare selector, matches preview)
     ============================================================ */
  if (step === "user-type") {
    return (
  ...
  return (
    <div className="mx-auto w-full max-w-[560px]">
      <button
        ...
        disabled={isPending}
        className="group mb-4 inline-flex items-center gap-2 text-[14.5px] font-semibold text-text-secondary transition-colors duration-200 ease-nc hover:text-ink disabled:opacity-60"
      >
      ...
      <div className="mb-4 text-center">
        ...
        <p className="mt-1.5 text-[15px] leading-[1.45] text-text-secondary">
          Setting up your {ROLE_LABEL[formData.role]} account.
        </p>
      </div>
      ...
      <div className="rounded-card border border-border-hair bg-surface p-[clamp(22px,2.4vw,30px)] shadow-card">
        <form onSubmit={handleSubmit} noValidate>
          ...
          <button
            type="submit"
            disabled={isPending}
            className="mt-2 inline-flex w-full items-center justify-center gap-2.5 rounded-full bg-ink px-7 py-4 text-[16px] font-semibold leading-none text-cream transition-[transform,background,box-shadow] duration-200 ease-nc hover:bg-black hover:shadow-card active:scale-[0.97] disabled:cursor-default disabled:opacity-65"
          >
            {isPending ? (
              <>
